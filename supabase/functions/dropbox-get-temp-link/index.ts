// dropbox-get-temp-link/index.ts
// Supabase Edge Function — gets a temporary download link for a Dropbox file.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Call Dropbox get_temporary_link
    ...
    if (!linkRes.ok) {
      ...
      console.error('Dropbox get_temporary_link error:', errText);
      return jsonResponse({ error: 'Dropbox API error' }, 502);
    }

    const linkData = await linkRes.json();

    return jsonResponse({ link: linkData.link as string });

  } catch (err) {
    console.error('dropbox-get-temp-link error:', err);
